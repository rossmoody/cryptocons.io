...
Cryptocons is written in Typescript and comes typed.
...
Once the package is installed, importing and using icons is pretty simple.

```bash
import { Binance, Coinbase } from 'cryptocons
```

Each icon is wrapped in a `forwardRef` that exposes the underlying SVG element. Any valid SVG property can be added.
...
**Badge** icons can be rendered with the `icon` property.
...
**Logo** icons can be rendered with the `icon` property.
...
The `createIcon` function is a convenience wrapper around the Icon component allowing you to achieve the same result with less effort. The default `viewBox` is `0 0 24 24` when using the `createIcon` function.
